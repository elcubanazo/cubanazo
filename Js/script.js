function loadCartForCurrentLocation() {
  const storageKey = typeof getCartStorageKey === "function"
    ? getCartStorageKey()
    : "cart_default";

  try {
    const storedLocationCart = localStorage.getItem(storageKey);
    if (storedLocationCart !== null) {
      const parsed = JSON.parse(storedLocationCart);
    ...
    if (legacyCart !== null) {
      ...
      const migratedCart = Array.isArray(parsedLegacyCart) ? parsedLegacyCart : [];
      localStorage.setItem(storageKey, JSON.stringify(migratedCart));
      localStorage.removeItem("cart");
      return migratedCart;
    }
  } catch (error) {
    console.warn("No se pudo cargar el carrito de la ubicación actual:", error);
  }

  return [];
}

function switchCartToLocation() {
  cart = loadCartForCurrentLocation();
  updateCartCount();
  if (typeof updateCart === 'function') updateCart();
}

let cart = loadCartForCurrentLocation();
let products = [];
...
// Escapa texto (nombre/descripcion de productos, packs, eventos) antes de
// insertarlo en innerHTML, para evitar XSS si ese texto llegara a contener
// HTML/JS (por ejemplo, un error humano al cargarlo desde el panel admin).
...
// valor va dentro de un atributo HTML entre comillas dobles (alt="...", data-x="...").
...
function initDetailFloatingPanel(product) {
  ...
  const imgSrc = product.imagenes && product.imagenes[0] ? `${getProductImageUrl(product.imagenes[0], 'thumb')}` : '';
...
function getProductAvailabilityState(product) {
  ...
  if (Object.prototype.hasOwnProperty.call(product, 'disponible')) {
...
function isProductStockAvailable(product) {
  if (!product || !getProductAvailabilityState(product)) return false;
  if (!isStockControlled(product)) return true;
  const available = getAvailableToAdd(product);
...
function findCurrentCartItemData(cartItem) {
  ...
  if (cartItem.pack) {
    const packId = storedData.id != null ? String(storedData.id) : null;
    if (packId && Array.isArray(packs)) {
      const packMatchById = packs.find((p) => p.id != null && String(p.id) === packId);
      if (packMatchById) return packMatchById;
    }
    if (storedData.nombre && Array.isArray(packs)) {
      const packMatchByName = packs.find((p) => p.nombre === storedData.nombre);
...
async function handleRouteChange() {
  ...
  if (decodedHash.startsWith("search=")) {
    if (backBtnWrapper) backBtnWrapper.style.display = "none";
    const searchTerm = decodedHash.substring(7).trim();
...
function initCarousel() {
  // Renderizar el carrusel estacional (tarjetas con productos aleatorios)
  // ANTES de cachear nodos, para que slides e indicadores existan ya.
  if (typeof renderSeasonalCarousel === "function") {
    try { renderSeasonalCarousel(); } catch (e) { console.warn("Carrusel estacional:", e); }
  }
...
function renderMainViewForCurrentRoute() {
  const detailVisible = document.getElementById('product-detail')?.style.display === 'block';
  const packsDetailVisible = document.getElementById('packs-detail')?.style.display === 'block';

  if (detailVisible || packsDetailVisible) {
    return;
  }
  ...
  if (hash.startsWith('category=')) {
    const categoryName = hash.substring(9);
    const filtered = categoryName === 'Todo' || !categoryName
      ? products
      : products.filter((p) => p.categoria === categoryName);
...
async function loadProducts(sourceUrl = null) {
  try {
    ...
    // Excluir la categoría "General" que técnicamente no existe
    categories = [
      "Todo",
      ...new Set(
        products
          .map((product) => product.categoria)
          .filter((cat) => normalizeString(cat || "").toLowerCase() !== "general")
      ),
    ];
    ...
    renderCategories();
    initPriceFilter();
    renderMainViewForCurrentRoute();

    // Refrescar el carrusel estacional con los productos actualizados
    // (re-elige productos aleatorios cuando cambia el catálogo en Firebase).
    if (typeof refreshSeasonalCarousel === "function") {
      try { refreshSeasonalCarousel(); } catch (e) { console.warn("Refresh carrusel estacional:", e); }
    }

    updateCartCount();
    refreshCartWithLiveCatalogData();
...
function filterByCategory(category) {
  ...
  // Ocultar detalle de producto antes de mostrar los resultados
  ...
  if (category !== "Todo") {
    ...
    // Mostrar solo los productos filtrados; ocultar el contenido promocional de inicio.
    hidePacksDetail();
    if (categoryCardSection) {
      categoryCardSection.style.display = "none";
    }
    if (categoriesCircleSection) {
      categoriesCircleSection.style.display = "none";
    }
    if (bannerContainer) {
    ...
    renderCategoryCard();
    if (categoriesCircleSection) {
      categoriesCircleSection.style.display = "block";
    }
    if (bannerContainer) {
...
function getSearchRouteUrl(searchTerm = "") {
  const url = new URL(window.location.href);
  if (url.pathname.startsWith("/p/")) url.pathname = "/";
  url.search = "";
  url.hash = searchTerm ? `search=${encodeURIComponent(searchTerm)}` : "";
  return url.href;
}

function searchProducts(showSuggestions = true, syncUrl = true) {
  const searchInput = document.getElementById("search-input");
  const productsContainer = document.getElementById("products-container");
  ...
  if (syncUrl) {
    ...
    if (window.location.href !== searchUrl) {
      window.history.replaceState({ search: searchTerm }, searchTerm ? `Buscar: ${searchTerm}` : "Inicio", searchUrl);
    }
  }
  ...
  if (searchTerm) {
    ...
    setFlashDealsSectionVisible(false);
    updateSearchResultsHeader(searchTerm);
  ...
  if (!searchTerm) {
    ...
    renderBestSellers();
    renderCategoriesCircle();
    productsContainer.classList.remove("has-no-results");
    hideNoResultsMessage();
    return;
  }
  ...
  if (filteredProducts.length > 0) {
    ...
    productsContainer.classList.add("has-no-results");
...
function clearSearch() {
  ...
  const homeUrl = getSearchRouteUrl();
  ...
  renderCategoriesCircle();
  renderBestSellers();
  document.getElementById("products-container")?.classList.remove("has-no-results");
  hideNoResultsMessage();
  clearSearchSuggestions();
}

function updateSearchResultsHeader(searchTerm) {
  ...
  if (!header) {
    ...
    productsContainer.parentNode.insertBefore(header, productsContainer);
  }

  header.innerHTML = `
    <span class="search-results-label">Resultados para</span>
...
function renderSearchSuggestions(suggestions) {
  ...
  container.innerHTML = suggestions.map((sugg, idx) => {
    ...
    if (sugg.type === 'product') {
      const p = sugg.data;
      imgSrc = p.imagenes?.[0] ? `${getProductImageUrl(p.imagenes[0], 'thumb')}` : 'Images/no-image.png';
      ...
      imgSrc = pack.imagenes?.[0] ? `${getPackImageUrl(pack.imagenes[0], 'thumb')}` : `Images/pack-placeholder.svg`;
...
function openCategoriesFromBottomNav() {
  const bottomNav = document.getElementById("bottom-nav");
  const sidebar = document.getElementById("sidebar");
  const categories = document.getElementById("sidebar-categories");
  const sidebarOverlay = document.getElementById("sidebar-overlay");
  if (!sidebar || !bottomNav || getComputedStyle(bottomNav).display === "none") return;
  ...
  requestAnimationFrame(() => {
    if (categories) {
      sidebar.scrollTo({ top: Math.max(0, categories.offsetTop - 16), behavior: "smooth" });
    ...
    <article class="event-content">
      ...
      <section class="event-section">
        <h3 class="event-section-title">
          <i class="fas fa-gift"></i> ${escapeHtml(eventData.nombre)}
        </h3>
      </section>
...
function getStoredEventId() {
  return localStorage.getItem('cubanazo_event_id');
}
function saveEventId() {
  if (evento && evento.id) {
    localStorage.setItem('cubanazo_event_id', evento.id.toString());
  }
}
    ...
    categoryProducts.forEach((product) => {
                          ...
                          <div class="variant-thumb ${
                            index === product.currentVariant ? "active" : ""
                              ...
                              <img src="${getProductImageUrl(
                                (variant.imagenes && variant.imagenes[0]) || '', 'thumb'
                              )}" alt="${
                            variant.variantName
      ...
      productEl.innerHTML = `
              <div class="product-image-container ${
                !displayProduct.disponibilidad ? "unavailable" : ""
                  ...
                  <div class="product-badges">
                      ...
                      ${
                        (displayProduct.aplicar_stock && isStockLow(displayProduct))
                          ? '<span class="badge stock-bajo"><i class="fas fa-exclamation-triangle"></i> Stock bajo</span>'
                          : ""
                      }
                      ${
                        // ===== BADGES DINÁMICOS =====
                    ...
                    <img src="${getProductImageUrl((displayProduct.imagenes && displayProduct.imagenes[0]) || '', 'thumb')}" 
                      class="product-image" 
              ...
              <div class="product-info">
                  ...
                  <div class="quantity-section" data-product-name="${escapeAttr(displayProduct.nombre)}">
...
function changeProductVariant(thumbElement, baseName, variantIndex, event) {
  ...
  productCard.querySelector(
    ".product-image"
  ).src = `${getProductImageUrl((variant.imagenes && variant.imagenes[0]) || '', 'thumb')}`;
  ...
  const badgesHTML = `
        ...
        ${
          (variant.aplicar_stock && isStockLow(variant))
            ? '<span class="badge stock-bajo"><i class="fas fa-exclamation-triangle"></i> Stock bajo</span>'
            : ""
        }
    `;
  ...
  flashDeals.forEach((product) => {
    ...
    card.innerHTML = `
      <div class="flash-deal-image-container">
        ...
        <img class="flash-deal-image" src="${(product.imagenes?.[0] || product.imagen) ? getProductImageUrl(product.imagenes?.[0] || product.imagen, 'thumb') : 'Images/product-placeholder.svg'}" alt="${escapeAttr(product.nombre)}" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='Images/product-placeholder.svg'">
      </div>
      <div class="flash-deal-info">
        <h3 class="flash-deal-title">${escapeHtml(product.nombre)}</h3>
...
function renderBestSellers() {
  ...
  if (!bestSellersScroll || !bestSellersSection) return;
  ...
  // Mostrarla únicamente en la página principal, nunca durante una búsqueda
  // ni mientras se está viendo una categoría filtrada.
  if (!document.getElementById("search-results-header") && isMainHomepageView()) {
    bestSellersSection.style.display = "block";
  }
  ...
  bestSellers.forEach((entry, index) => {
    ...
    const imageSrc = isPack
      ? ((displayItem.imagenes?.[0] || displayItem.imagen) ? getPackImageUrl(displayItem.imagenes?.[0] || displayItem.imagen, 'thumb') : 'Images/pack-placeholder.svg')
      : ((displayItem.imagenes?.[0] || displayItem.imagen) ? getProductImageUrl(displayItem.imagenes?.[0] || displayItem.imagen, 'thumb') : 'Images/product-placeholder.svg');
    const basePrice = Number(displayItem.precio) || 0;
    const discount = Number(displayItem.descuento) || 0;
    const isOnSale = displayItem.oferta === true && discount > 0;
    const priceValue = isPack
      ? getPackFinalPrice(displayItem)
      : (isOnSale ? basePrice * (1 - discount / 100) : basePrice);
            ...
            <div class="best-seller-image-container">
                <span class="best-seller-badge">
                 ...
                 <img src="${imageSrc}" 
                     ...
                     onerror="this.onerror=null;this.src='Images/${isPack ? 'pack-placeholder.svg' : 'product-placeholder.svg'}'">
            </div>
            ...
            <div class="best-seller-info">
                ...
                <div class="best-seller-price">
                    ${
                      isOnSale
                        ? `
                        <span class="best-seller-price-original">${basePrice.toFixed(2)} <img src="Images/Zelle.svg" alt="Zelle" class="currency-icon price-xs"></span>
                        <span class="best-seller-discount">-${Math.round(discount)}%</span>
                    `
                        : ""
                    }
    ...
    card.innerHTML = `
      <div class="category-circle">
        <img 
          ...
          loading="lazy"
          decoding="async"
          onerror="this.onerror=null;this.src='Images/categories-placeholder.jpg'"
        >
...
async function showProductDetail(arg) {
  ...
  const canAddDetail = isProductStockAvailable(product);
  const detailAvailable = getAvailableToAdd(product);
  const disableDetailIncrease = Number.isFinite(detailAvailable) && detailAvailable <= 1;
                  ...
                  .map(
                    ...
                    <div class="variant-thumb ${
                      index === variantIndex ? "active" : ""
                        ...
                        <img src="${getProductImageUrl(v.imagenes[0], 'thumb')}" alt="${
                      v.variantName
                    }" loading="lazy" decoding="async">
  ...
  // Mostrar badge de stock bajo si aplica
  if (isStockLow(product)) {
    badges.push('<span class="detail-badge stock-bajo"><i class="fas fa-exclamation-triangle"></i> Stock bajo</span>');
  }
  ...
  const specs = [
    ...
    `<li><strong>Disponibilidad</strong> ${
      product.aplicar_stock ? (isStockLow(product) ? 'Stock bajo' : 'Stock disponible') : 'Stock disponible'
    }</li>`,
                          ...
                          suggested.cleanName || suggested.nombre
                            ...
                            <div class="suggested-details">
                                ...
                                <button class="add-to-cart-mini" onclick="addToCart('${
                                  encodeURIComponent(suggested.nombre)
                                }', false, event)">
  ...
  detailContainer.innerHTML = `
    ...
    product.cleanName
                    ...
                    ? `<div class="detail-thumbnails">
                        ${product.imagenes
                          ...
                          <div class="detail-thumbnail ${
                            index === 0 ? "active" : ""
                          }" onclick="changeMainImage('${encodeURIComponent(
                              img
                            )}', this)">
                              <img src="${getProductImageUrl(
                                img, 'thumb'
                              )}" alt="${product.cleanName} - foto ${
                              index + 1
                            }" loading="lazy" decoding="async">
                          </div>
                        `
            ...
            <div class="detail-info">
                ...
                <div class="detail-rating" id="detail-rating-${product.id}" data-product-id="${product.id}">
                ...
                <div class="quantity-section">
                    ...
                    <div class="quantity-controls">
                        <button class="quantity-btn" id="detail-decrease-btn" onclick="adjustDetailQuantity(-1, event)">
                            <i class="fas fa-minus"></i>
                        </button>
                        <span class="quantity-display" id="detail-quantity">1</span>
                        <button class="quantity-btn" id="detail-increase-btn" onclick="adjustDetailQuantity(1, event)">
                            <i class="fas fa-plus"></i>
                        </button>
                    </div>
                </div>

                <button class="add-to-cart-btn ${!canAddDetail ? "disabled" : ""}" 
                        onclick="addToCart('${encodeURIComponent(product.nombre)}', true, event)"
                        ${!canAddDetail ? "disabled" : ""}>
                    <i class="fas fa-${!canAddDetail ? "lock" : "cart-plus"}"></i>
                    ${!canAddDetail ? "Producto Agotado" : "Añadir a la cesta"}
                </button>
                </aside>
  ...
  setTimeout(() => {
    ...
    try { initSuggestedProductsCarousel(); } catch (e) { console.warn('No fue posible inicializar el carrusel de sugeridos:', e); }
  }, 100);
}
...
function initSuggestedProductsCarousel() {
  ...
  // Drag con mouse solo en dispositivos no táctiles (desktop)
  if (!isTouchDevice) {
  ...
  // En dispositivos táctiles se deja el scroll nativo (overflow-x + scroll-snap)
  // sin manipular scrollLeft manualmente, para evitar el conflicto que causaba el salto.
...
function changeMainImage(encodedImgSrc, thumbEl) {
  const imgSrc = safeDecodeURIComponent(encodedImgSrc);
  const mainImg = document.getElementById("main-product-image");
  if (mainImg) {
    mainImg.src = `${getProductImageUrl(imgSrc, 'detail')}`;
  ...
  if (thumbEl) {
    const container = thumbEl.closest(".detail-thumbnails");
    if (container) {
      container
  ...
  const bundleItemsHTML = bundleProducts
    .map((product, index) => {
      ...
      const imageSrc = product?.imagenes?.[0]
        ? `${getProductImageUrl(product.imagenes[0], 'thumb')}`
...
function addBundleToCart(productNames, event) {
  ...
  const normalizedNames = Array.isArray(productNames)
    ? [...new Set(productNames.filter(Boolean).map((name) => safeDecodeURIComponent(String(name))))]
    : [safeDecodeURIComponent(String(productNames))];
  ...
  for (const decodedName of normalizedNames) {
    const product = products
      .flatMap((p) => (p.isGrouped ? p.variants : [p]))
    ...
    const available = getAvailableToAdd(product);
    if (!(product.disponibilidad !== false) || (Number.isFinite(available) && available <= 0)) {
      showCartNotification("Stock no disponible", 1, "error");
      return;
    }
    ...
    if (existingItem) {
      // available is computed above as remaining units that can be added
...
function addToCart(productName, fromDetail = false, event) {
  ...
  const decodedName = safeDecodeURIComponent(productName);
  const product = products
    .flatMap((p) => (p.isGrouped ? p.variants : [p]))
    .find((v) => v.nombre === decodedName);
  ...
  if (fromDetail) {
    ...
    const quantityElement = productCard
      ? productCard.querySelector(".quantity-display")
      : null;
    quantity = quantityElement ? parseInt(quantityElement.textContent) || 1 : 1;
  }
  ...
  // Ajustar cantidad si existe control de stock
...
function renderEmptyCartRecommendations() {
  ...
  const cardsHTML = recommendedPacks.map((pack) => {
    ...
    const imagePath = `${getPackImageUrl(pack.imagen, 'thumb')}`;

    return `
      <article class="empty-cart-rec-card">
        <img class="empty-cart-rec-image" src="${imagePath}" alt="${escapeAttr(pack.nombre)}" loading="lazy" onerror="this.onerror=null;this.src='Images/pack-placeholder.svg'">
        <div class="empty-cart-rec-info">
          <div class="empty-cart-rec-badges">${badges.join('')}</div>
          <h4>${escapeHtml(pack.nombre)}</h4>
          <p>${description}</p>
...
function updateCart() {
  ...
  if (cart.length === 0) {
    ...
    cart.forEach((item, index) => {
      ...
      const imageSrc = isPack 
        ? `${getPackImageUrl(itemData.imagen, 'thumb')}`
        : `${getProductImageUrl(itemData.imagenes && itemData.imagenes[0] ? itemData.imagenes[0] : '', 'thumb')}`;
      ...
      const priceUpdatedNotice = (isAvailable && item._priceUpdated)
        ? '<p class="cart-item-price-updated"><i class="fas fa-info-circle"></i> El precio se actualizó desde que lo agregaste.</p>'
        : '';
      ...
      itemEl.innerHTML = `
                ...
                <img src="${imageSrc}" alt="${escapeAttr(itemData.nombre)}">
                <div class="cart-item-info">
                    <p>${escapeHtml(itemData.nombre)}</p>
...
function refreshCartWithLiveCatalogData() {
  ...
  cart.forEach((item) => {
    const storedData = item.pack || item.product;
    if (!storedData || !Number.isFinite(Number(item.quantity)) || Number(item.quantity) <= 0) {
      changed = true;
      return;
    }

    const isPack = Boolean(item.pack || item.isPack);
    if (isPack && (!Array.isArray(packs) || packs.length === 0)) {
      refreshedCart.push(item);
      return;
    }

    const liveData = findCurrentCartItemData(item);
    if (!liveData || !productIsAvailable(liveData)) {
      changed = true;
      return;
    }

    const oldPrice = Number(storedData.precio) || 0;
    const newPrice = Number(liveData.precio) || 0;
    if (oldPrice !== newPrice) {
      item._priceUpdated = true;
    }

    if (item.pack) {
      if (item.pack !== liveData) {
        item.pack = liveData;
        changed = true;
      }
    } else if (item.product) {
      if (item.product !== liveData) {
...
function addPackToCart(packName, event) {
  ...
  const decodedName = safeDecodeURIComponent(packName);
...
function adjustQuantity(btn, change, productName, event) {
  ...
  if (quantityElement) {
    ...
    const product = products.flatMap(p => p.isGrouped ? p.variants : [p]).find(v => v.nombre === productName);
    if (change > 0) {
      const available = getAvailableToAdd(product);
      if (Number.isFinite(available)) {
        const nextQuantity = Math.min(quantity + change, available);
        if (nextQuantity === quantity) {
          showCartNotification('Has alcanzado el máximo stock disponible', 1, 'error');
        }
...
function adjustDetailQuantity(change, event) {
  ...
  if (quantityElement) {
    ...
    quantityElement.textContent = quantity;
    updateDetailQuantityButtons();
  }
}

function updateDetailQuantityButtons() {
  const quantityElement = document.getElementById("detail-quantity");
  const increaseBtn = document.getElementById("detail-increase-btn");
  if (!quantityElement || !increaseBtn) return;

  const quantity = parseInt(quantityElement.textContent) || 1;
  const available = getAvailableToAdd(currentProduct);
  const disableIncrease = Number.isFinite(available) ? quantity >= available : false;
...
function updateCartCount() {
  ...
  const count = cart.reduce((acc, item) => {
    ...
    return Number.isFinite(quantity) && quantity > 0 ? acc + quantity : acc;
  }, 0);
  if (countElement) countElement.textContent = count;
  if (mobileCountElement) mobileCountElement.textContent = count;
}
...
function isStockControlled(product) {
  ...
  if (typeof product.aplicar_stock === 'boolean') {
    return product.aplicar_stock;
  }

  if (typeof product.aplicar_stock === 'string') {
    return product.aplicar_stock.toLowerCase() === 'true' || product.aplicar_stock === '1';
  }

  return Number(product.aplicar_stock) === 1;
}
...
function getAvailableToAdd(displayProduct) {
  if (!displayProduct) return 0;
  if (!isStockControlled(displayProduct)) return Number.POSITIVE_INFINITY;
  const stock = Number(displayProduct.stock ?? 0);
  const inCart = getProductCartQuantity(displayProduct.nombre);
  return Math.max(0, stock - inCart);
}

function isStockLow(displayProduct){
  if (!isStockControlled(displayProduct)) return false;
  const stock = Number(displayProduct.stock ?? 0);
  return stock > 0 && stock < STOCK_LOW_THRESHOLD;
}
...
function getProductQuantityHTML(productName, displayProduct) {
  ...
  const available = getAvailableToAdd(displayProduct);
  const low = isStockLow(displayProduct);

  if (quantity === 0) {
    // Mostrar botón de agregar. Si no hay unidades disponibles (control de stock), deshabilitar.
    const disabled = !(displayProduct.disponibilidad !== false) || (Number.isFinite(available) && available <= 0);
    const icon = disabled ? 'lock' : 'cart-plus';
    const label = !(displayProduct.disponibilidad !== false) ? 'Agotado' : 'Añadir a la cesta';

    return `
      <button class="add-to-cart ${disabled ? 'disabled' : ''}" 
              onclick="addToCart('${encodeURIComponent(displayProduct.nombre)}', false, event)"
              ${disabled ? 'disabled' : ''}>
    ...
    // Mostrar contador con botones + y -; deshabilitar + si llegamos al máximo disponible
    ...
    const disableIncrease = Number.isFinite(available) && available <= 0;

    return `
      <div class="quantity-counter">
        <button class="${decreaseClass}" 
                onclick="decreaseProductQuantity('${encodeURIComponent(displayProduct.nombre)}', event)"
                title="${isTrash ? 'Eliminar del carrito' : 'Disminuir cantidad'}">
          <i class="fas fa-${decreaseIcon}"></i>
        </button>
        <span class="quantity-display">${quantity}</span>
        <button class="quantity-btn increase-btn" 
                onclick="increaseProductQuantity('${encodeURIComponent(displayProduct.nombre)}', event)"
...
function increaseProductQuantity(productName, event) {
  ...
  if (cartItem) {
    // Verificar disponibilidad según aplicar_stock
    const product = cartItem.product || cartItem.pack;
    const available = getAvailableToAdd(product);
    if (Number.isFinite(available) && available <= 0) {
      showCartNotification('Stock no disponible', 1, 'error');
      return;
...
document.addEventListener("click", (e) => {
  const cart = document.getElementById("cart");
  const cartButtons = document.querySelectorAll(".cart-btn, .bottom-nav-item:nth-child(4)");
  const sidebar = document.getElementById("sidebar");
  const menuToggle = document.getElementById("menu-toggle");
  const sidebarTriggers = document.querySelectorAll("#menu-toggle, .bottom-categories-btn");
  ...
  if (
    ...
    !Array.from(cartButtons).some(button => button.contains(e.target))
  ) {
    closeCart();
  }
  ...
  if (
    ...
    !Array.from(sidebarTriggers).some(button => button.contains(e.target))
  ) {
    closeSidebar();
  }
});
...
function openWhatsApp() {
  const selectedUbicacion = typeof getSelectedUbicacion === "function"
...
function renderCategoryCard() {
  ...
  // Mostrar solo packs disponibles
  const displayPacks = getAvailablePacks().slice(0, 4);
  ...
  displayPacks.forEach((pack) => {
    ...
    item.innerHTML = `
      <div class="category-card-image-container">
        <img src="${imagePath}" 
             alt="${escapeAttr(pack.nombre)}" 
             class="category-card-image"
             loading="lazy" 
             decoding="async"
             onerror="this.onerror=null;this.src='Images/pack-placeholder.svg'">
      </div>
      <div class="category-card-label">${escapeHtml(pack.nombre)}</div>
    `;
...
function getAvailablePacks() {
  return Array.isArray(packs)
    ? packs.filter((pack) => pack && pack.disponible !== false)
    : [];
}
...
function showAllPacks() {
  ...
  const currentHashDecoded = safeDecodeURIComponent(window.location.hash.substring(1) || '');
...
function renderPacksDetail() {
  ...
  if (availablePacks.length === 0) {
    ...
    if (window.location.hash === "#packs") {
      window.history.replaceState(null, "Inicio", getSearchRouteUrl());
    }
    return;
  }
  ...
  availablePacks.forEach(pack => {
    ...
    packsHTML += `
      <article class="pack-card" style="cursor: pointer;" onclick="showPackDetail('${encodeURIComponent(pack.nombre)}', event)">
        ...
        <h2 class="pack-name">${escapeHtml(pack.nombre)}</h2>
        <p class="pack-desc">${escapeHtml(pack.descripcion)}</p>
        ...
        <button class="pack-btn" 
                onclick="addPackToCart('${encodeURIComponent(pack.nombre)}', event)"
...
function showPackDetail(packName, event) {
  ...
  if (!pack.disponible) badges.push('<span class="detail-badge no-stock"><i class="fas fa-ban"></i> Stock no disponible</span>');
  ...
  const imagePath = `${getPackImageUrl(pack.imagen, 'detail')}`;
  ...
  detailContainer.innerHTML = `
    <div class="detail-container">
      ...
      <div class="detail-info">
        <h1 class="detail-title">${escapeHtml(pack.nombre)}</h1>
        ...
        <button class="add-to-cart-btn ${!pack.disponible ? 'disabled' : ''}" 
                onclick="addPackToCartFromDetail('${encodeURIComponent(pack.nombre)}', event)"
                ${!pack.disponible ? 'disabled' : ''}>
          <i class="fas fa-${!pack.disponible ? 'lock' : 'cart-plus'}"></i>
          ${!pack.disponible ? 'Pack no disponible' : 'Agregar al carrito'}
        </button>
        
        <div class="product-description">
          <h4 class="description-title"><i class="fas fa-align-left"></i> Descripción</h4>
          <div class="description-content">
            <p>${escapeHtml(pack.descripcion)}</p>
          </div>
        </div>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (searchInput) {
    ...
    // Evento nativo 'search': se dispara al tocar "Buscar/Ir" en el teclado móvil
    // para input type="search". Cierra el teclado tras ejecutar la búsqueda.
    ...
    // Cerrar el teclado también al tocar el botón de lupa

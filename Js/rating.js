...
let API_BASE = 'https://backend-szp7.onrender.com';
(function detectApiBase() {
  try {
    const host = window.location.hostname;
    const port = window.location.port;
    if (host === 'https://backend-szp7.onrender.com') {
      API_BASE = '';
      return;
    }
  } catch (e) {
...
// -----------------------------------------------------------------------
// Resumen en bloque: en vez de una petición GET /product-ratings POR
// PRODUCTO (miles de invocaciones al backend en un catálogo grande), se
// pide UNA sola vez /api/ratings-summary con el promedio/votos de TODOS
// los productos. Cada producto individual solo vuelve a golpear el backend
// si de verdad falta en ese resumen (caso raro) o si el usuario vota.
// -----------------------------------------------------------------------
let summaryFetchPromise = null;
let summaryLoadedAt = 0;

function applyRatingsSummaryObject(summary) {
  Object.keys(summary || {}).forEach((productId) => {
    const entry = summary[productId] || {};
    ratingCache.set(productId, {
      avg: Number(entry.avgRating) || 0,
      votes: Number(entry.totalVotes) || 0,
      ts: Date.now()
    });
  });
}

async function fetchRatingsSummaryOnce() {
  const now = Date.now();
  if (summaryFetchPromise && (now - summaryLoadedAt) < RATING_CACHE_TTL_MS) {
    return summaryFetchPromise;
  }

  if (window.__CUBANAZO_BOOTSTRAP_RATINGS__) {
    summaryLoadedAt = now;
    applyRatingsSummaryObject(window.__CUBANAZO_BOOTSTRAP_RATINGS__);
    summaryFetchPromise = Promise.resolve(window.__CUBANAZO_BOOTSTRAP_RATINGS__);
    return summaryFetchPromise;
  }

  if (window.__CUBANAZO_BOOTSTRAP_PROMISE__) {
    summaryLoadedAt = now;
    summaryFetchPromise = window.__CUBANAZO_BOOTSTRAP_PROMISE__
      .then((boot) => {
        const summary = boot && boot.ratings ? boot.ratings : {};
        applyRatingsSummaryObject(summary);
        return summary;
      })
      .catch((err) => {
        console.warn('[rating] No se pudo cargar el bootstrap de ratings; se usarán fetch individuales:', err);
  ...
  const base = API_BASE || '';
  summaryLoadedAt = now;
  summaryFetchPromise = fetch(`${base}/api/ratings-summary`)
    .then((res) => {
      if (!res.ok) throw new Error(`Error al obtener resumen de ratings (status: ${res.status})`);
      return res.json();
    })
    .then((data) => {
      if (!data || data.success === false) throw new Error((data && data.message) || 'Error al obtener resumen de ratings');
      const summary = data.ratings || {};
      applyRatingsSummaryObject(summary);
      return summary;
    })
    .catch((err) => {
      console.warn('[rating] No se pudo cargar el resumen de ratings en bloque, se usará fetch individual como respaldo:', err);
      summaryFetchPromise = null; // permitir reintento en la próxima llamada
      return {};
    });
...
export async function initRatings(selector = '[id^="product-"]') {
  // Trae el resumen de TODOS los ratings en una sola petición antes de
  // procesar los nodos, así la primera pasada ya pinta el promedio real
  // (sin placeholder en 0) y el resto del flujo de abajo no dispara
  // peticiones individuales para productos que ya vinieron en el resumen.
  await fetchRatingsSummaryOnce();
  ...
  nodes.forEach(node => {
    ...
    // Si ya vino en el resumen en bloque, pinta el promedio y los votos
    // reales de una vez. Solo se usa placeholder en 0 para productos
    // realmente nuevos sin votos (no están todavía en ratings_summary).
    const cached = ratingCache.get(productId);
    if (cached) {
      updateProductUI(productId, cached.avg, cached.votes);
    } else {
